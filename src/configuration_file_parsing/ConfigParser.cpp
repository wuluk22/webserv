...
void ConfigParser::processLocationBlock(std::ifstream &config_file, std::string working_line, TokenCounter &token_counter, size_t &current_line, LocationBlock *loc_directive = NULL) {
	...
	while (std::getline(config_file, working_line)) {
		...
		if (is_token_valid(working_line_splitted[0], B_LOC) && working_line_splitted.size() == 2) {
			...
			break;
		} else if (is_token_valid_multiple(working_line_splitted[0], l_params) 
			|| is_token_valid_multiple(working_line_splitted[0], c_params)) {
			token_counter.incrementToken(working_line_splitted[0]);
			if (token_counter.getTokenCount("cgi_path") > 1 || token_counter.getTokenCount("cgi_extensions") > 1 || token_counter.getTokenCount("root") > 1 || token_counter.getTokenCount("return") > 1) {
...
void ConfigParser::processServerBlock(std::ifstream &config_file, std::string working_line, size_t &current_line) {
	...
	while (std::getline(config_file, working_line)) {
		...
		working_line_splitted = split(working_line, ' ');
		if (is_token_valid(working_line, B_LOC) && working_line_splitted.size() == 2) {
			processLocationBlock(config_file, working_line, token_counter, current_line);
			...
			token_counter.incrementToken(working_line_splitted[0]);
			if (token_counter.getTokenCount("root") > 1) {
				std::cerr << ERROR_HEADER << TOKEN_REPEATED << AL << current_line << std::endl;
				throw ConfigException();
			}
			...
			std::cerr << ERROR_HEADER << INVALID_TOKEN << AL << current_line << std::endl;
			throw ConfigException();
		}
	}
	config_file.seekg(last_position);
...
void ConfigParser::parseConfigurationFile(std::ifstream &config_file) {
	...
	while (std::getline(config_file, working_line)) {
		...
		if (working_line_splitted[0] == B_SERVER && working_line.size()) {
			...
			std::cerr << ERROR_HEADER << TOKEN_POSITION_MISMATCH << AL << std::endl;
			throw ConfigException();
		}
	}
}
